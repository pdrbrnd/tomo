...
struct AZW3WriterTests {
    ...
    @Test func nullRecordHasMOBIMagicAndKF8FileVersion() {
        ...
        let firstRecOffset = (UInt32(bytes[78]) << 24)
            | (UInt32(bytes[79]) << 16)
            | (UInt32(bytes[80]) << 8)
            | UInt32(bytes[81])
        ...
        let fileVersion = (UInt32(bytes[fileVersionAt]) << 24)
    ...
    @Test func textRecordCountMatchesPalmDocHeader() {
        ...
        let firstRecOffset = Int((UInt32(bytes[78]) << 24)
            | (UInt32(bytes[79]) << 16)
            | (UInt32(bytes[80]) << 8)
            | UInt32(bytes[81]))
    ...
    private func parseRecordOffsets(_ bytes: [UInt8]) -> [Int] {
        ...
        for i in 0..<recordCount {
            let start = 78 + i * 8
            let offset = (UInt32(bytes[start]) << 24)
