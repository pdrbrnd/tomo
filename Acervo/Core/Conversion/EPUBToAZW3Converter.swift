...
/// Bridges Acervo's EPUB pipeline to the AZW3 writer. Reads an EPUB
/// off disk via `EPUBSource`, hands the resulting `BookManifest` to
/// `AZW3Writer`, and writes the produced bytes into the conversion
/// scratch directory.
///
/// Lives outside `Acervo/Core/Conversion/AZW3/` so the writer stays
/// free of EPUB and Acervo concerns. When the writer is extracted to
/// a standalone package, this file stays in Acervo.
nonisolated struct EPUBToAZW3Converter: FormatConverter {
    ...
    func convert(source: URL, into scratchDir: URL) async throws -> URL {
        ...
        let outputURL = scratchDir
            .appending(component: source.deletingPathExtension().lastPathComponent
                + "." + output.rawValue)
