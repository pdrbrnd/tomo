...
/// variant Acervo produces. Sits immediately after the 16-byte PalmDoc
...
nonisolated struct KF8Header: Sendable {
    ...
    /// Index fields that Acervo doesn't populate. Spec convention is
    ...
    func encoded() -> Data {
        ...
        assert(w.count == Self.length, "KF8Header serialised to \(w.count) bytes, expected \(Self.length)")
        return w.data
    }
}
